#ifndef REFERENCE_LINE_CREATOR_H_
#define REFERENCE_LINE_CREATOR_H_

#include "rclcpp/rclcpp.hpp"
#include "base_msgs/msg/referline.hpp"
#include "base_msgs/msg/referline_point.hpp"
#include "nav_msgs/msg/path.hpp"
#include "geometry_msgs/msg/pose_stamped.hpp"
#include "cmath"

#include "config_reader.h"
#include "curve.h"
#include "reference_line_smoother.h"

namespace Planning
{
    using base_msgs::msg::Referline;
    using base_msgs::msg::ReferlinePoint;
    using nav_msgs::msg::Path;
    using geometry_msgs::msg::PoseStamped;

    class ReferencelineCreator // 创建参考线
    {
    public:
        ReferencelineCreator();
        Referline create_reference_line(const Path &global_path,
                                        const PoseStamped &target_point); // 创建参考线，输入为全局路径对象，输出为参考线对象
        Path reference_to_rviz();               // 参考线for rviz
        void init_reference_line();                                       // 初始化参考线，输入为全局路径对象，输出为参考线对象
    public:
        inline Referline refer_line() const
        {
            return refer_line_;
        }       // 获取参考线对象
        inline Path refer_to_rviz() const { return refer_line_rviz_; }      // 获取参考线for rviz对象
        inline int match_point_index() const { return match_point_index_; } // 获取匹配点下标
        inline int front_index() const { return front_index_; } // 获取前视点下标
        inline int back_index() const { return back_index_; }               // 获取后

    private:
        std::unique_ptr<ConfigReader> reference_line_config_;             // 配置
        Referline refer_line_;                                           // 参考线
        Path refer_line_rviz_;                                           // 参考线for rviz
        std::shared_ptr<ReferenceLineSmoother> reference_line_smoother_; // 参考线平滑器
        int last_match_point_index_ = -1;                                // 上次匹配点下标
        int match_point_index_ = -1;                                     // 匹配点下标
        int front_index_ = -1;                                     // 前视点下标
        int back_index_ = -1;                                            // 后视点下标
    };
} // namespace Planning
#endif // REFERENCE_LINE_CREATOR_H_
